...
class PostBloc extends Bloc<PostEvent, PostState> {
  ...
  Future<void> _onPostFetched(PostFetched event, Emitter<PostState> emit) async {
    ...
    try {
      ...
      emit(posts.isEmpty ? state.copyWith(hasReachedMax: true) : state.copyWith(
        status: PostStatus.success,
  ...
  Future<List<Post>> _fetchPosts([int startIndex = 0]) async {
    final response =  await httpClient.get(Uri.https('jsonplaceholder.typicode.com', '/posts', <String, String>{'_start': '$startIndex', '_limit': '$_postLimit'}));
